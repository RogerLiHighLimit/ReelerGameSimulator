...
namespace ReelerGameSimulator.AnvilServiceMock.Logic
{
    ...
    {
        internal static List<PayoutResult> MatchLineWins(GameState st, EngineConfiguration config)
        {
        ...
        internal static long CalculatePayoutAmount(GameState gameState, EngineConfiguration gameConfig, PayoutResult pay)
        {
            long totalAmount = 0;

            if (pay.Amount < 0M)
                throw new SimulatorException(string.Format("Invalid payout amount: {0}.", pay.Amount));
            ...
            switch (pay.Type)
            {
                ...
                default:
                    throw new SimulatorException($"Pay table type: {pay.Type}");
            }
        ...
        internal static long ConvertDecimalToLongSafely(decimal orignial)
        {
            ...
            {
                throw new SimulatorException("Losing financil data");
            }
